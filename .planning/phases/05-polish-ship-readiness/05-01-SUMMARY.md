...
## Task 4 Completion (continuation, 2026-08-24)

User created/pushed deploy repo (`jmaillot/www`) and bound custom domain `www.jeremymaillot.fr` (CNAME record + Pages custom-domain setting). astro.config site updated to `https://www.jeremymaillot.fr`; `public/CNAME` added; `<Font>` components wired (commit 9343345) closing the PERF-03 carry-forward.

**Live URL checks (all PASS against https://www.jeremymaillot.fr):**
- Routes `/`, `/projects/`, `/skills/`, `/contact/` → 200; unknown path → branded 404
- `/.nojekyll` → 200 (served root); `/_astro/BaseLayout.*.css` → 200; `/_astro/fonts/*.woff2` → 200; favicon → 200
- Live HTML contains self-hosted `@font-face` rules; zero googleapis/gstatic refs
- Identity renders from content layer on live pages (jeremymaillot@gmail.com, github.com/jmaillot)
- Zero D-07 placeholder strings on all 5 routes
- HTTP/2 over TLS

**Human spot-check list (presented to user for final confirmation):**
1. Mobile menu: focus trapped, Escape closes, focus restored, aria-expanded syncs
2. prefers-reduced-motion: reveals appear without animation
3. Skip link jumps to main content

Self-Check amended: PARTIAL → PASSED (deployment verified live).
